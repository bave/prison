...
do
	echo "ssh 192.168.1.${i} sh /mnt/aufs/root/git/prison/eval/scale_test.sh"
	ssh -o HashKnownHosts=yes 192.168.1.${i} "export LD_LIBRARY_PATH=/lib:/usr/lib:/usr/local/lib; chroot /mnt/aufs env"
	ssh -o HashKnownHosts=yes 192.168.1.${i} "export LD_LIBRARY_PATH=/lib:/usr/lib:/usr/local/lib; chroot /mnt/aufs sh /root/git/prison/eval/scale_test.sh -p $PROCESS -d $NODE_NUM"
	i=`expr $i + 1`
done
